...
# CROSSOVER EXPERIMENT 2
# each row in child is selected randomly from the list of rows in each  parent
def crossover_ind(parent, parent2):
    ...
    for row in range(len(child)):
        r_row = choice(numbers)
        numbers.remove(r_row)
        r_parent = choice([r1, r2])
        child[r_row] = r_parent[r_row]
    return [j for i in child for j in i]


# MUTATE INDIVIDUAL BY SWAPPING VALUES of 2 AVAILABLE CELL'S
def mutate_ind(individual, cells, m):
    for i in cells:
        if random() < m :
            aval = cells[:]  # available cells for box
            f = choice(aval)
            aval.remove(f)
            s = choice(aval)
            f_value = individual[f]  # value of first chosen index in individual
            s_value = individual[s]  # value of second chosen index in individual
            individual[f] = s_value
            individual[s] = f_value
...
def experiment(p,m,s, cells, lis):
    ...
    best_fit = 0
    generation = 0
    while best_fit != 1.0 and generation < 10000:
...
mutation_rates_grid1 = [0.01, 0.04, 0.04, 0.04]
...
def results_grid2():
    ...
    for i in range(len(pop_sizes)):
        ...
        for runs in range(5):
            start = time.time()
            experiment(pop_size, mutation_rate, selection_proportion, available_cells2, l2)
            end = time.time()
